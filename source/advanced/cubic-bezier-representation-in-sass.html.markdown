...
categories: advanced, experiment
author: Tim Severien and Hugo Giraudel
summary: Tim Severien and Hugo Giraudel built a little tool to visualize cubic Bézier functions in nothing but Sass.
---

# Cubic Bézier Representation in Sass

Everything started when [Tim Severien](https://twitter.com/timseverien) showed me a pen where he managed to display a Bezier curve with nothing but Sass. After a couple of hours of fiddling, between his math/geometry knowledge and my love to Sass, we came up with a neat little tool to display a Cubic Bezier function in Sass.

Here's the story but before anything else you might want to catch up on some basic knowledge regarding Bezier functions. Tim, please.
...
In computer graphics, creating curves was quite a complex task. In 1959, physiscist and mathematician Paul de Casteljau, who worked at Citroën, developed an algorithm that helped creating curves for car designs. Mathematician Pierre Bézier adopted the algorithm to design the curvatures for Renault. In 1962 Pierre widely publicised what we now know as the Bézier curve.

The curve is widely used across many fields in computer graphics. Most digitally produced curvatures are made using this algorithm. Think of your car, phone or the font you’re reading. It was also adapted in less visible fields, like easing transitions.

The most known implementation is vector paths in 2D and moddeling in 3D graphic software. A path usually consists of many points connected by lines. Each line gets one or two “control points” to control the curvature.

To create Bézier curves, a basic understanding of linear interpolation is required first.
...
Imagine you have two numbers, 3 and 7. Between these numbers, what number is in the middle? “The difference between 7 and 3 is 4. Adding the half of 4 to 3 makes 5.” That’s exactly how linear interpolation works, but instead of dealing with numbers, we’re dealing with points in a 2D or 3D space, thus you have to do this two or three times.
...
*Note: if you haven't already, I highly recommand you to watch [this 4-minutes video](http://vimeo.com/106757336) about the way Bézier curves are drawn by a computer. Fair warning: ah-ha moment triggerer.*
...
Such a function can be reprensented on a 2-axis plan with the transition/animation progression along the Y axis, and the time along the X axis. A curve is then drawed on the graph, representing the timing function.
...
Basically exactly like the `cubic-bezier` function from CSS. We can also add an extra argument to pass a map of options if defaults are not convenient enough (see below to have an explanation of the available options):
...
Luckily, Sass provides basic mathematical operations like additions, subtraction, multiplication and division. Enough to create some basic curves. Because CSS lacks a drawing API to draw lines, I chose to use box-shadow on one element to mark some points along the path, generated using Sass.
...
However, Sass doesn’t can’t do arithmetic operations on maps or lists. Linear interpolation only works with numbers, so an extra function is required to lerp on each axis, assuming a point is a list of two numbers:
...
All of this is quite pointless. That being said, you could still use it to have an idea of what a cubic function would look like (although [this tool](http://cubic-bezier.com/) from Lea Verou would probably be easier); it's always more convenient that a series of 4 floats, right?

Also, this is the kind of experiment that's both really fun to do, and quite insightful. Not only does it help understanding what's happening under the hood (Cubic Bézier in this example), but it's also a good practice to write clean and powerful API (in Sass in this case).

Hope you enjoyed the experiment. You can play with the code on CodePen:
